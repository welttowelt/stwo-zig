...
### Direct four-lane Blake2s word transposition

Commit `98e395a` removes compiler-generated byte deinterleaving from the shared four-lane Blake2s
path. On little-endian targets, four 64-byte messages are bitcast to words and transposed with
explicit 4x4 vector shuffles before compression; the eight output words are transposed back to four
canonical digests in the same way. Big-endian targets retain the scalar little-endian load and
digest conversion. The ten fixed Blake2s rounds are also compile-time unrolled. The parent wrapper
fell from approximately 300 to 99 AArch64 instructions.

A seven-run ReleaseFast parent microbenchmark improved from 23.489 to 26.628 million parents per
second, or 13.36 percent. Separate isolation measured 12.42 percent from word transposition and
2.36 percent from round unrolling. Reverse-order, five-run medians used ten warmups and 21 measured
samples per run:

| Workload | Before prove (ms) | After prove (ms) | Prove-time gain |
| --- | ---: | ---: | ---: |
| Wide Fibonacci `log10x8` | 1.823125 | 1.792625 | 1.67% |
| Wide Fibonacci `log14x8` | 11.260792 | 10.852125 | 3.63% |
| XOR `log14` | 11.677167 | 11.155084 | 4.47% |
| Plonk `log14` | 12.241792 | 11.736083 | 4.13% |

Every measured artifact remained byte-exact. Final Wide Fibonacci, XOR, and Plonk artifacts at
logs 10 and 14 passed the pinned Rust Stwo oracle. Debug tests, focused ReleaseFast Blake2s tests,
formatting, API parity, source conformance, and a `powerpc64` big-endian compile check passed. The
optimization is shared by CPU Merkle leaves, parents, and proof of work; it does not branch on the
AIR or benchmark identity.
