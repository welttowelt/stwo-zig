...
This artifact is the input to the Fib25k per-component loop. It does not claim base-trace, AIR,
interaction, proof, or verifier parity on its own. The next conformance slice must:

1. feed these exact bytes to the pinned Rust base-trace exporter;
