...
pub const MetalCommitBackend = struct {
    ...
    pub const quadratic_recurrence_min_cells: usize = 1 << 20;

    pub fn admitsQuadraticRecurrenceTrace(row_count: usize, column_count: usize) bool {
        const cells = std.math.mul(usize, row_count, column_count) catch return false;
        return column_count >= 2 and column_count <= 256 and
            cells >= quadratic_recurrence_min_cells;
    }

    pub fn fillQuadraticRecurrenceTrace(
        columns: [][]@import("stwo_core").fields.m31.M31,
        log_n_rows: u32,
        recipe: [7]u32,
    ) !void {
