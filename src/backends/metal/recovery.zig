...
pub const BufferAccess = struct {
    ...
    pub fn fromResidentArena(resident: *arena_plan.ResidentArena) BufferAccess {
        return .{ .context = resident, .bytes_fn = residentBytes };
    }

    fn residentBytes(raw: *anyopaque, binding: arena_plan.Binding) ![]u8 {
        const resident: *arena_plan.ResidentArena = @ptrCast(@alignCast(raw));
        return resident.bytes(binding);
    }
};
...
/// Coalesces a multi-output Metal operation (LogUp, LDE, Merkle, quotient, or
/// FRI) behind every logical output id. The operation is dispatched once per
/// schedule tick even though the arena owns each output independently.
...
test "Metal recovery: aligned spill roundtrip and corruption detection" {
...
test "Metal recovery: recipe registry is fail closed" {
...
test "Metal recovery: grouped protocol operation dispatches once per tick" {
    const Context = struct {
        calls: usize = 0,
