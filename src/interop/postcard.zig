...
test "postcard: proof serialize/deserialize roundtrip" {
    const xor = @import("../examples/xor.zig");
    const alloc = std.testing.allocator;
    const Hasher = proof_wire.Hasher;

    const pcs_config = pcs.PcsConfig{
        .pow_bits = 0,
    ...
    var output = try xor.prove(alloc, pcs_config, statement);
    defer output.proof.deinit(alloc);

    // Serialize
    var out_buf = std.ArrayList(u8).empty;
    defer out_buf.deinit(alloc);
    try serializeProof(Hasher, out_buf.writer(alloc), output.proof);
    const encoded = out_buf.items;
    ...
    defer out_buf2.deinit(alloc);
    try serializeProof(Hasher, out_buf2.writer(alloc), decoded);

    try std.testing.expectEqualSlices(u8, encoded, out_buf2.items);

    // Verify that the decoded proof is still valid
    try xor.verify(alloc, pcs_config, output.statement, decoded);
}
