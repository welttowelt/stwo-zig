//! AIR component for the AUIPC (Add Upper Immediate to PC) instruction.
//!
//! Instructions: AUIPC (1 op).
//!
...
//!   - enabler is boolean.
//!   - result = pc + (imm_u << 12).
//!   - result = result_lo + result_hi * 2^16 (range check).
//!   - rd_val = result.
...
pub const Columns = trace.AuipcColumns;
...
/// Evaluate the auipc AIR constraints.
pub fn evaluate(eval: *ExprEvaluator) !void {
    ...
    // result = pc + imm_u * 2^12
    const expected_result = try arena.baseAdd(pc, try arena.baseMul(imm_u, shift_12));
    try eval.addConstraint(try arena.extBase(
    ...
    // Range check 8_8 for result
    try eval.writeLogupFrac(.{
        .numerator = try arena.extFromBase(enabler),
        .denominator = try arena.extSub(z, try arena.extFromBase(result_recon)),
    });
