...
const OwnedSn2DecommitBindings = struct {
    ...
    fn deinit(self: *OwnedSn2DecommitBindings, allocator: std.mem.Allocator) void {
...
fn collectSn2DecommitBindings(
    ...
    const fri_geometry = protocol_recipes.FriGeometry.init(fri_start_log) catch return Error.InvalidBindingSize;
    ...
    for (0..Sn2Counts.decommit_fri_trees) |round| {
        const fri_leaf_log = try fri_geometry.leafLog(round);
...
fn friStartLog(quotient: arena_plan.Binding) !u32 {
...
fn executeStreamingCommitment(
    ...
    for (group_descriptors, 0..) |descriptor, group_index| {
        ...
        if (std.process.hasEnvVarConstant("STWO_ZIG_SN2_LOG_COMMIT_SOURCE_DIGESTS"))
            try logCommitSourceDigests(resident_arena, coefficient_cursor, group);
        if (tree_index == 2 and
        ...
        for (4..lifting_log + 1) |evaluation_log_usize| {
            ...
            if (group_index == 48 and std.process.hasEnvVarConstant("STWO_ZIG_SN2_REPEAT_COMMIT_LDE")) {
                const first_digest = sampleCommitOutputs(resident_arena, outputs.items, evaluation_log);
                ...
                const second_digest = sampleCommitOutputs(resident_arena, outputs.items, evaluation_log);
                std.debug.print(
    ...
    if (std.process.hasEnvVarConstant("STWO_ZIG_SN2_LOG_COMMIT_DIGESTS"))
        try logBindingDigest(resident_arena, "commit_leaf", 0, leaf_state);
    ...
    if (std.process.hasEnvVarConstant("STWO_ZIG_SN2_LOG_COMMIT_DIGESTS")) {
        for (retained, 0..) |layer, layer_index|
            try logBindingDigest(resident_arena, "commit_retained", layer_index, layer);
    }
