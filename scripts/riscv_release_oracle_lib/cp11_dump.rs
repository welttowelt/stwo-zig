...
impl prover::components::ComponentVisitor for RelationTupleVisitor<'_> {
    fn visit<E: FrameworkEval>(
        ...
        _claimed_sum: stwo::core::fields::qm31::QM31,
    ) {
        ...
        for relation in RELATION_NAMES {
            let selected: Vec<&RelationTrackerEntry> = entries
                .iter()
                .filter(|entry| entry.relation == relation)
                .collect();
            let digest = digest_entries(selected);
            println!(
                "component_relation={component_name}/{relation} entries={} digest={}",
                digest.count, digest.digest,
            );
        }
...
fn dump_relation_tuples(result: runner::RunResult) {
    ...
    println!("schema=riscv-relation-tuples-v1");
    ...
    // Keep PublicData in this mode so the instrumentation follows the same
    // production construction path as proving, even though tuples themselves
    // do not depend on relation challenges or public compensation.
