...
def prog_alu_basic() -> list[int]:
    """The historical alu_test.elf program, regenerated byte-identically."""
    return [ADDI(1, 0, 10), ADDI(2, 0, 20), ADD(3, 1, 2), SUB(4, 2, 1), ECALL()]
...
def prog_mem_ls() -> list[int]:
    return [
        LUI(1, 0x00200000),  # data region base
        LUI(2, 0x12345000),
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.scratch is not None:
        ...
        return update(args.scratch) if args.update else gate(args.scratch)
    with tempfile.TemporaryDirectory() as tmp:
        scratch = Path(tmp)
        return update(scratch) if args.update else gate(scratch)
