...
| RISC-V | Interaction transcript | The production claim phase now mixes the canonical statement, roots, main claim, a domain-separated shard manifest, a schema-v3 fixed 10-bit PoW nonce, twelve independent relation pairs, and the canonical interaction claim/root. A helper-level byte-event recorder covers this order, but a full orchestration and independent-process trace receipt remains open. | Stark-V uses the same shared prefix and release PoW value but has no shard-manifest extension; its generated constant also drops PoW to 1 bit in debug builds. | The shard extension blocks byte-identical proof-wire parity; missing full-path and cross-process mutation evidence blocks adapter release. |
...
| RISC-V | Signed `MULH` carry relation | Zig reproduces the pinned witness and keeps the `mulh` family trace-incompatible: `MULH(-1, -1)` yields canonical `carry_4 = 2,139,096,569`, which cannot enter the required `[0, 2^11)` lookup. | Stark-V adds `sign * 128` to each top operand byte before the carry recurrence. Its test named `test_prove_opcode_mulh` loads MULHU output, so no signed-high proof regression exercises the invalid carry. | Blocks signed `MULH`/`MULHSU`, the complete RV32IM statement, CP-04/05/11, and adapter release. Close only against a corrected Rust oracle with signed proof-and-verify vectors and renewed relation receipts. |
