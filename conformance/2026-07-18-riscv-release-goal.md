...
**Current accepted baseline:** `d38e1f041fc7f013acadd4115cb3723e9bdd9ad1`.
The live pinned-Rust receipt at that revision passes 7 of 11 shared boundaries.
It is diagnostic parity evidence, not a release acceptance receipt.
...
At baseline `d38e1f041fc7f013acadd4115cb3723e9bdd9ad1`, a clean build of the pinned
Rust Stark-V commit and the Zig candidate agrees at these live boundaries:
...
The receipt remains non-passing because these four boundaries are not yet live:
...
The first two are extraction and comparison work over production buffers. The
last two are proof-placement work: they must expose the exact signed relation
tuples and cumulative per-component accumulators generated by the production
AIR. A self-comparison, a Zig-only digest, a reconstructed shadow trace, or two
equally omitted sides of a bus is not evidence.
...
| --- | --- | --- | --- |
| CP-00 Fail closed | IN_PROGRESS | Closed registry and staged CLI through `d38e1f04` | Typed candidate-only `--experimental`; complete negative CLI/decoder/registry matrix |
...
| CP-02 Execute | IN_PROGRESS | Live decode and execution boundaries pass at `d38e1f04` | Exact 45-opcode manifest, complete edge corpus, and corrected signed-MULH oracle |
| CP-03 Witness | IN_PROGRESS | Family schemas exist; live boundary is unimplemented | Production-buffer per-family row vectors and layout digest agree with Rust |
...
| CP-06 Memory/hash/tables | IN_PROGRESS | Live roots and Poseidon2 permutation vectors pass | Exact Merkle-parent hash, Poseidon2 IO, range, bitwise, and memory AIR closure |
...
| CP-09 CLI | IN_PROGRESS | Staged prove/independent-verify loop at `3c035713` | Installed candidate/promoted prove, verify, bench, help, and failure matrix |
| CP-10 Artifact | IN_PROGRESS | Staged schema and atomic path exist | Version/security policy enforcement, owned statement data, and hostile decoding matrix |
| CP-11 Rust oracle | FAIL | Fresh `d38e1f04` receipt is 7/11 | All four missing live boundaries pass with candidate-bound strict evidence |
...
| CP-13 Release gate | IN_PROGRESS | Base enforcing gate at `0745516b`; final evidence not accepted | Strict candidate and promoted controllers pass locally and in clean CI |
