...
- `src/backends/metal/resident_fri_transaction.zig`;
...
Final candidate `a94b70436d2f` has all manual source files at or below 850
lines, source-conformance reports no new violations, and the ReleaseFast Metal
benchmark builds. A post-split log16 check measured 11.913167 ms, preserved
61,470 proof bytes and SHA-256
...
tree. The first final-tree run landed just outside significance:
...
All G1--G5 gates pass over 15 paired rounds. This final verdict, not the
earlier source commit's verdict, is the packaged claimed evidence.
