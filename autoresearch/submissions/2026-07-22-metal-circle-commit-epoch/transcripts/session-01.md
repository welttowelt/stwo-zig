...
## CI closure correction

PR #76's focused Metal lane passed, but the first aggregate-macOS lane failed after all 70 runtime tests passed because `src/backends/metal/combined_commit.zig` was not named in the aggregate product closure. Adding it to `build_support/products/aggregate.zig` made the exact ReleaseSafe lane and 411-source closure pass locally, but the refreshed autoresearch harness correctly rejected that policy file as outside the editable set. The policy edit was therefore removed. The helper moved to `src/backends/metal/runtime/combined_commit.zig`, which is covered by the existing authorized runtime prefix, and its relative imports were adjusted. The exact CI command again passed locally without a policy diff.

The final clean candidate `09bfd9bd8b97` then received a fresh green S3 objective verdict: 173.083 -> 160.541 ms, ratio 0.9304, CI [0.9057, 0.9555], five winning rounds, request ratio 0.972172, RSS ratio 0.999981, energy ratio 0.987632, and the same canonical/oracle-valid 86,383-byte proof. G1-G5 all pass, including the editable-path gate.
