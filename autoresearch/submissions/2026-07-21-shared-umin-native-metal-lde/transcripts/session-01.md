...
Candidate `7e70f1a70ba4` is measured against exact predecessor
...
Both exceed their noise-derived promotion thresholds. CPU energy was 4.46%
lower and Metal energy 3.18% lower; peak RSS and proof size stayed within
budget. All timed proofs verified, the arms were byte-identical, and the pinned
Rust oracle accepted both fixed proofs (`e6609d...` huge and `f845568c...`
xlarge). G1--G5 and all 13 automatic regression guards passed for each verdict.

The first PR CI pass exposed an architecture-specific portability failure:
Linux x86_64's RISC-V product built successfully but its `branch_fib` proof
reported `ConstraintsNotSatisfied`. The exact proof passed on AArch64, and all
other CPU and Metal CI lanes were green. The only cross-platform behavioral
change was the new fixed/native vector reduction and vectorized `QM31.add`.
Those operations are now compile-time gated to AArch64; all other targets use
the exact predecessor select/scalar operations. This keeps the measured ARM
machine code intact while restoring x86 semantics. Field laws, the exact local
RISC-V proof, and the static x86_64 Linux product build pass after the change.
